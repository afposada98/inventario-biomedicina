            ...
            <table id="example2" class="table display table-bordered">
                ...
                <tbody>
                    ...
                    while ($f = mysqli_fetch_array($query)) { ?>
                        <tr>
                            <td class="orden-dato"><span><?php echo $f['id_marca'] ?></span></td>
                            <td class="orden-dato"> <?php echo $f['descripcion'] ?> </td>
                                <?php if ($perfil == 3 || $perfil == 2)  { ?>                     
                            <td class="text-center"> <a href="javascript:void(0);" type="button"
                                onclick="cargar_ajax1('modal_ooo', 'modal_modificar_marca.php?id=<?= $f['id_marca'] ?>');" title="Modificar Marca" 
                                data-toggle="modal" data-target="#modal_ooo"> <i class="fas fa-pen-square" style="color: slateblue; font-size:34px;"></i>
                            </td>                             
                            <td class="text-center">
                                        <a style="margin: 0;" title="Eliminar Marca"
                                            class="btn btn-danger btn-sm" onclick="ConfirmDelete(<?php echo $f['id_marca'];?>)">
  ...
  function ConfirmDelete(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        window.location = "eliminar_marca.php?id="+id;
      } else {       
