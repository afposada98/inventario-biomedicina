            ...
            <table id="example2" class="table display table-bordered">
                ...
                <tbody>
                    ...
                    while ($f = mysqli_fetch_array($query)) { ?>
                        <tr>
                            <td class="orden-dato"><span><?php echo $f['id_tipo'] ?></span></td>
                            <td class="orden-dato"> <?php echo $f['descripcion'] ?> </td>
                                <?php if ($perfil == 3 || $perfil == 2)  { ?>                     
                            <td class="text-center"> <a href="javascript:void(0);" type="button"
                                onclick="cargar_ajax1('modal_ooo', 'modal_modificar_tipo_equipo.php?id=<?= $f['id_tipo'] ?>');" title="Modificar Tipo de Equipo" 
                                data-toggle="modal" data-target="#modal_ooo"> <i class="fas fa-pen-square" style="color: slateblue; font-size:34px;"></i>
                            </td>                             
                            <td class="text-center">
                                        <a style="margin: 0;" title="Eliminar Tipo de Equipo"
                                            class="btn btn-danger btn-sm" onclick="ConfirmDelete(<?php echo $f['id_tipo'];?>)">
...
<div class="modal-dialog" style="margin-top: 9rem;">
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">Agregar Tipo de Equipo </h5>
        </div>

            <form action="registrar_tipo_equipo.php" method="post">
                <div class="modal-body">

                    <div class="form-group col-md-12">
                        <label class="descripcion">Nombre del Tipo de Equipo</label>
  ...
  function ConfirmDelete(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        window.location = "eliminar_tipo_equipo.php?id="+id;
      } else {       
