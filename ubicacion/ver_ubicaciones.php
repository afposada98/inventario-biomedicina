...
<body style="margin-bottom: 20px;">
    ...
    <div class="container">
        ...
        <div class="row tipo">
                ...
                <?php 
                    $sql = "SELECT ubi.*, area.descripcion AS area FROM ubicacion AS ubi INNER JOIN area ON area.id_area=ubi.id_area";
            ...
            <table id="example2" class="table display table-bordered">
                ...
                <tbody>
                    ...
                    while ($f = mysqli_fetch_array($query)) { ?>
                        <tr>
                            <td class="orden-dato"><span><?php echo $f['id_ubicacion'] ?></span></td>
                            <td class="orden-dato"> <?php echo $f['area'] ?> </td>
                            <td class="orden-dato"> <?php echo $f['descripcion'] ?> </td>
                                <?php if ($perfil == 3 || $perfil == 2)  { ?>                     
                            <td class="text-center"> <a href="javascript:void(0);" type="button"
                                onclick="cargar_ajax1('modal_ooo', 'modal_modificar_ubicacion.php?id=<?= $f['id_ubicacion'] ?>');" title="Modificar Ubicación" 
                            ...
                            <td class="text-center">
                                        <a style="margin: 0;" title="Eliminar Producto"
                                            class="btn btn-danger btn-sm" onclick="ConfirmDelete(<?php echo $f['id_ubicacion'];?>)">
...
<div class="modal-dialog" style="margin-top: 3rem;">
    <div class="modal-content">
        <div class="modal-header">
            ...
            <form action="registrar_ubicacion.php" method="post">
                <div class="modal-body">       
  ...
  function ConfirmDelete(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        window.location = "eliminar_ubicacion.php?id="+id;
      } else {       
